...
const Contact = ({ darkMode, toggleDarkMode }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSubmitted(true);
            setError(err.text || 'Failed to send message. The email may not exist or our server encountered an error.');
        } finally {
            setIsLoading(false);
            setTimeout(() => {
                setClosing(true);
            ...
            <div className={`app-container ${darkMode ? 'dark' : 'light'}`}>
                ...
                <section id="contact" className={`contact-section ${darkMode ? 'dark' : 'light'}`}>
                    ...
                    <div className="contact-grid">
                        {[
                            ...
                            { icon: <FaEnvelope />, title: "Email", content: "vishaldevre528@gmail.com" },
                            { icon: <FaMapMarkerAlt />, title: "Location", content: "Chh.Sambhajinagar, Maharashtra, India" }
                        ].map((item, index) => (
                            <div
                    ...
                    <div className="lets-talk">
                        ...
                        <div className="social-links">
                            {[
                                { icon: <FaInstagram />, url: "https://www.instagram.com/vishall.1211/" },
                                { icon: <FaLinkedin />, url: "https://www.linkedin.com/in/vishaldevre/" },
                ...
                {showForm && (
                    <div className={`modal-overlay ${closing ? 'closing' : ''}`} onClick={handleClose}>
                        <div className={`modal-content ${darkMode ? 'dark' : 'light'} ${closing ? 'closing' : ''}`} onClick={(e) => e.stopPropagation()}>
                            {isLoading ? (
                                ...
                                <div className={`submission-feedback ${submitStatus === 'success' ? 'success' : 'error'} ${submitStatus === 'error' ? 'shake-animation' : ''}`}>
                                    <div className="feedback-icon">
                                        {submitStatus === 'success' ? <FaCheck /> : <FaTimesCircle />}
                                    </div>
                                    <h3>{submitStatus === 'success' ? 'Thank You!' : 'Delivery Failed'}</h3>
                                    <p>
